...
def prepareProtocol(resolved, context, sourceRoot, sourceCommit):
  ...
  match = re.search(r'''__version__\s*=\s*["']([^"']+)["']''', versionFile.read_text(encoding='utf-8'))
  if not match:
    raise BuildConfigError('Cannot determine protocol version from source version file')
  version = match[1]
